...
class WebBaseController extends BaseController
{
    ...
    public function _initialize()
    {
        ...
        $news = $Portal_Category->field('id,parent_id,name,description,more')->where(['parent_id'=>11,'status'=>1,'delete_time'=>0])->order('list_order')->select()->toArray();
        View::share('site_info', $siteInfo);
        View::share('news', $news);
        foreach ($news as $n) {
            $n['articles'] = getNewList($n['id']);
            $newList[] = $n;
        }
        $applyClasses = Db::name('portal_category')->where(['parent_id'=>23])->select()->toArray();
        $class = '';
        foreach ($applyClasses as $item) {
            $class .= '<option value="'.$item['name'].'" data-id="'.$item['id'].'">'.$item['name'].'</option>';
        }
        $children = $Portal_Category->where(['delete_time' => 0,'id'=>21])->value('more');
        View::share('children', json_decode($children,true));
        View::share('applyClasses', $class);
        View::share('newList', $newList);
        View::share('hotClass', hotClassNew());
//        View::share('hotTag', hotTag('',10));
//        $links = Db::name('link')->where(['status'=>1])->order('list_order')->select()->toArray();
        View::share('links', Links('link'));
        View::share('hotTag', Links('tag'));
        View::share(cmf_get_option('footer'));
    }
