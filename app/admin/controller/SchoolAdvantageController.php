...
class SchoolAdvantageController extends AdminBaseController
{
    /*
     * 列表页
     */
...
//            $data['more']['video']['file_urls'] = isset($data['more']['video']['file_urls']) && !empty($data['more']['video']['file_urls'])?:'';
        ...
        $data['type'] = 'schoolAdvantage';
        $data['admin'] = session('name');
        $check_re = $this->validate($data,$rule,$message);
        if($check_re!==true) $this->error($check_re);
        $SchoolAdvantage = new SchoolAdvantageModel();
        $SchoolAdvantage->isUpdate(false)->create($data);
    ...
    public function edit()
    {
        ...
        $SchoolAdvantage = new SchoolAdvantageModel();
        $banner = $SchoolAdvantage->where(['id'=>$id])->find()->toArray();
...
//            $data['more']['video']['file_urls'] = isset($data['more']['video']['file_urls']) && !empty($data['more']['video']['file_urls'])?:'';
    ...
    public function delete()
    {
        ...
        $SchoolAdvantage = new SchoolAdvantageModel();
        if(!empty($id) && $SchoolAdvantage->where(['id'=>$id,'type' => 'schoolAdvantage'])->delete() || !empty($ids) && $SchoolAdvantage->where(['id'=>['IN',implode(',',$ids)],'type' => 'schoolAdvantage'])->delete())
